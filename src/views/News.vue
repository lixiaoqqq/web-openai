      ...
      article
        ...
        header.post-header(:class="{'video': news.post_type == 'video'}")
          .label.article-info(v-if="news.post_type !== 'video' && news.column && !promotion[news.column.id]")
            ...
            .article-time(v-if="news.post_type !== 'pure_video'") {{news.reading_time}}min read
          ...
          .user-info
            ...
            template(v-if="userInfo && userInfo.roles && userInfo.roles.indexOf('admin') > -1")
              a.edit(:href="`https://admin.geekpark.net/posts/new?id=${news.id}`") 编辑
              a.edit(:href="`https://main_test.geekpark.net/api/v1/posts/${news.id}/duokan_style`" target="_blank") 导出电子书用HTML

        #article-body
          //- 非纯视频文章展示
          .topic-cover(v-if="news.post_type === 'text'")
            img#topic-cover(:src="news.cover_url" v-if="news.cover_url")
            .tips
              i.iconfont.icon-quotes
             
              span 摘4444
              <iframe frameborder="0" src="https://v.qq.com/txp/iframe/player.html?vid=a3502528mev" allowFullScreen="true"></iframe>
            p {{news.abstract}}
          .article-content(v-if="news.post_type !== 'pure_video'" v-html="news.content")
          //- 纯视频文章展示
          .pure-video-wrpper(v-if="news.post_type === 'pure_video'")
            .article-content(v-html="news.video_code")
            pre.working-personnel(v-if="news.working_personnel" v-html="news.working_personnel")
            pre.pure-video-abstract(v-if="news.abstract" v-html="news.abstract")
        //- .article-source
        //- .ad-if
      ...
      api.get(`wechat/js_config?request_url=${encodeURIComponent(window.location.href)}`).then(function(res) {
        ...
        wx.ready(function() {
          ...
          wx.updateAppMessageShareData({ 
            ...
            // imgUrl: 'https://imgslim.geekpark.net/geekpark-icon-196-black.png', // 分享图标
            imgUrl: 'https://summit-img.geekpark.net/gpk_web/geekpark-icon-196-black-1.png', // 分享图标
          })
          // “分享到朋友圈”及“分享到 QQ 空间”
        ...
        console.log("================= echat/js_config err: ", err);
        this.$message.error(err.toString())
      });
    }
